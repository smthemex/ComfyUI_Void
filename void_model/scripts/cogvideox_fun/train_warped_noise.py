...
from videox_fun.data.dataset_image_video_warped import (ImageVideoControlDataset,
                                                ImageVideoDataset,
                                                ImageVideoSampler,
                                                get_random_mask)
...
def parse_args():
    ...
    parser.add_argument(
        "--use_warped_noise",
        action="store_true",
        help="Use warped noise from Go-with-the-Flow instead of random Gaussian noise"
    )
    parser.add_argument(
        "--warped_noise_degradation",
        type=float,
        default=0.2,
        help="Maximum degradation level for warped noise (0.0-1.0). 0.0=pure warped noise, 1.0=pure Gaussian."
    )
    parser.add_argument(
        "--warped_noise_probability",
        type=float,
        default=1.0,
        help="Probability of using warped noise vs random Gaussian per batch (0.0-1.0). 1.0=always warped, 0.5=50/50 split, 0.0=always random."
    )
    parser.add_argument(
        "--validation_epochs",
        type=int,
    ...
    parser.add_argument(
        '--trimask_zeroout_removal',
        action='store_true',
        help='Whether to zero out the removal region of the video with trimasks.'
    )
    parser.add_argument(
        '--binarize_mask',
        action='store_true',
        help='Whether to binarize the mask.'
    )
    parser.add_argument(
        '--use_vae_mask',
        action='store_true',
        help='Whether to use vae to encode mask.'
    )
    parser.add_argument(
        '--use_quadmask',
        action='store_true',
        help='Whether to use 4-value quadmask [0, 63, 127, 255] instead of 3-value trimask [0, 127, 255].'
    )
    parser.add_argument(
        '--ablation_binary_mask',
        action='store_true',
        help='Ablation: remap quadmask [0,63,127,255] to binary [0,127] where [0,63]→0 and [127,255]→127.'
    )
    parser.add_argument(
        '--shuffle_mask',
        action='store_true',
        help='Whether to shuffle the mask.'
    )
    parser.add_argument(
...
def main():
    ...
    transformer3d = CogVideoXTransformer3DModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="transformer",
        use_vae_mask=args.use_vae_mask if hasattr(args, 'use_vae_mask') else False,
    )
    ...
    if args.transformer_path is not None:
        ...
        if args.use_vae_mask or args.shuffle_mask:
            # Handle channel dimension mismatch when using VAE mask
            # Old model: 32 channels (16 video + 16 mask)
            # New model with VAE mask: 48 channels (16 video + 16 VAE-mask + 16 masked-video)
            old_weight = state_dict['patch_embed.proj.weight']
            old_bias = state_dict['patch_embed.proj.bias'] if 'patch_embed.proj.bias' in state_dict else None
            old_total_dim = old_weight.size(1)  # Should be 264 (32ch * 8 + 8)
            new_total_dim = transformer3d.state_dict()['patch_embed.proj.weight'].size(1)  # Should be 384 (48ch * 8)

            if new_total_dim > old_total_dim:
                print(f"[INFO] Expanding weights from {old_total_dim} to {new_total_dim} dims for use_vae_mask")
                print(f"[INFO] Old shape: {old_weight.shape}, New shape: {transformer3d.state_dict()['patch_embed.proj.weight'].shape}")

                # Create new expanded weight tensor
                new_weight = torch.zeros_like(transformer3d.state_dict()['patch_embed.proj.weight'])

                # The input structure for 32ch model: [video_latents(128), mask_latents(128), pos_embed(8)]
                # The input structure for 48ch model: [video_latents(128), vae_mask(128), masked_video(128), pos_embed(8)]
                # 128 = 16 channels * 8 (patch_size)

                # Copy video latents weights (first 128 dims)
                new_weight[:, :128] = old_weight[:, :128]

                # For VAE mask channels (128:256), use the old mask weights as initialization
                # This makes sense as both represent mask information
                new_weight[:, 128:256] = old_weight[:, 128:256]

                # For masked video channels (256:384), intelligently initialize:
                # Option 1: Copy video weights (since masked video is similar to video)
                # new_weight[:, 256:384] = old_weight[:, :128]

                # Option 2: Average of video and mask weights (since masked video combines both)
                new_weight[:, 256:384] = (old_weight[:, :128] + old_weight[:, 128:256]) / 2.0

                # Copy positional embedding weights if they exist (last 8 dims in old model)
                if old_total_dim > 256:  # Has positional embeddings
                    new_weight[:, -8:] = old_weight[:, -8:]

                state_dict['patch_embed.proj.weight'] = new_weight

                # Handle bias if it exists
                if old_bias is not None and 'patch_embed.proj.bias' in transformer3d.state_dict():
                    # Bias doesn't need special handling, just use the same bias
                    state_dict['patch_embed.proj.bias'] = old_bias

                print(f"[INFO] Weight expansion complete. Ready for VAE mask training.")
    ...
    if args.gradient_checkpointing:
        transformer3d._set_gradient_checkpointing(transformer3d, value=True)
    ...
    train_dataset = ImageVideoDataset(
        ...
        trimask_zeroout_removal=args.trimask_zeroout_removal,
        use_quadmask=args.use_quadmask,
        ablation_binary_mask=args.ablation_binary_mask,
    )
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            # Warped noise support
            if args.use_warped_noise:
                new_examples["warped_noise"] = []
            ...
            for example in examples:
                if args.random_ratio_crop:
                    ...
                    if args.train_mode == "void":
                        mask_pixel_values = torch.from_numpy(example["input_condition"]).permute(0, 3, 1, 2).contiguous()
                        mask = torch.from_numpy(example["mask"]).permute(0, 3, 1, 2).contiguous()
                        pixel_values = torch.cat([pixel_values, mask_pixel_values, mask], dim=1)
                    ...
                    transform_spatial = transforms.Compose([
                        ...
                        transforms.RandomCrop([int(x) for x in random_sample_size]),
                    ])
                    ...
                    transform_spatial = transforms.Compose([
                        ...
                        transforms.RandomCrop(closest_size),
                    ])
                ...
                if args.train_mode == "void":
                    pixel_values = transform_spatial(pixel_values)
                    new_examples["pixel_values"].append(transform_value(pixel_values[:, :3, :, :]))
                    new_examples["mask_pixel_values"].append(transform_value(pixel_values[:, 3:6, :, :]))
                    new_examples["mask"].append(pixel_values[:, 6:7, :, :])

                    # Requantize mask if using quadmask mode to handle interpolation artifacts
                    if args.use_quadmask:
                        mask = new_examples["mask"][-1]
                        # Values are in [0, 1] range after /255 normalization
                        # Requantize to discrete levels: 0, 63/255 (~0.247), 127/255 (~0.498), 1.0
                        mask = torch.where(mask <= 31/255.0, 0.0, mask)
                        mask = torch.where((mask > 31/255.0) & (mask <= 95/255.0), 63/255.0, mask)
                        mask = torch.where((mask > 95/255.0) & (mask <= 191/255.0), 127/255.0, mask)
                        mask = torch.where(mask > 191/255.0, 1.0, mask)
                        new_examples["mask"][-1] = mask
                else:
                    new_examples["pixel_values"].append(transform_value(transform_spatial(pixel_values)))

                new_examples["text"].append(example["text"])

                # Transform and collect warped noise if available
                if args.use_warped_noise and "warped_noise" in example:
                    wn = example["warped_noise"]  # Shape: (T, C, H, W) in pixel space
                    print(f"[COLLATE DEBUG] Original warped_noise shape: {wn.shape}")

                    # Apply SAME spatial transform as pixel_values (resize + crop)
                    wn_transformed = transform_spatial(wn)

                    # Keep as-is (don't normalize - it's already Gaussian noise)
                    # Unlike pixel_values which go through /255 and mean/std normalization,
                    # Gaussian noise should stay at mean≈0, std≈1

                    print(f"[COLLATE DEBUG] After transform: {wn_transformed.shape}")
                    new_examples["warped_noise"].append(wn_transformed)
                ...
                if args.train_mode in ["inpaint", "inpaint-unmasked"]:
                    ...
                    if args.train_mode == "inpaint":
                        ...
                        mask_pixel_values = new_examples["pixel_values"][-1]
            ...
            # Stack warped noise if available
            if args.use_warped_noise and "warped_noise" in new_examples and len(new_examples["warped_noise"]) > 0:
                print(f"[COLLATE DEBUG] Before stack: {len(new_examples['warped_noise'])} items")
                print(f"[COLLATE DEBUG] First item shape: {new_examples['warped_noise'][0].shape}")
                new_examples["warped_noise"] = torch.stack(new_examples["warped_noise"])
                print(f"[COLLATE DEBUG] After stack: shape={new_examples['warped_noise'].shape}")
                print(f"[COLLATE DEBUG] batch_video_length={batch_video_length}, pixel_values shape={new_examples['pixel_values'].shape}")
    ...
    # DATALOADER DEBUG BEGIN
    # batch_sampler.sampler.generator = torch.Generator().manual_seed(args.seed)
    ...
    #         gif_name = '-'.join(text.replace('/', '').split()[:10]) if not text == '' else f'{step}-{idx}'
    ...
    #         masks = rearrange(batch['mask'].cpu(), "b f c h w -> b c f h w")
    #         for idx, (pixel_value, mask_pixel_value, mask, text) in enumerate(zip(pixel_values, mask_pixel_values, masks, texts)):
    #             pixel_value = pixel_value[None, ...]
    #             mask_pixel_value = mask_pixel_value[None, ...]
    #             mask = torch.tile(mask[None, ...], (1, 3, 1, 1, 1)) * 2. - 1.  # will be rescale back to 0~1 with RGB
    #             data_tuple = torch.cat([mask_pixel_value, mask, pixel_value], dim=-1)
    #             save_videos_grid(data_tuple, f"{args.output_dir}/sanity_check/tuple_{gif_name[:10] if not text == '' else f'{step}-{idx}'}.gif", rescale=True)
    #     if step == 1: break
    # quit()
    # DATALOADER DEBUG END
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if epoch == first_epoch and step == 0:
                ...
                if args.train_mode != "normal":
                    ...
                    # Commenting out visualization due to size mismatch issues
                    # for idx, (pixel_value, mask_pixel_value, mask, text) in enumerate(zip(pixel_values, mask_pixel_values, masks, texts)):
                    #     pixel_value = pixel_value[None, ...]
                    #     mask_pixel_value = mask_pixel_value[None, ...]
                    #     mask = torch.tile(mask[None, ...], (1, 3, 1, 1, 1)) * 2. - 1.  # will be rescale back to 0~1 with RGB
                    #     data_tuple = torch.cat([mask_pixel_value, mask, pixel_value], dim=-1)
                    #     save_videos_grid(data_tuple, f"{args.output_dir}/sanity_check/tuple_{gif_name[:10] if not text == '' else f'{global_step}-{idx}'}.gif", rescale=True)
            ...
            with accelerator.accumulate(transformer3d):
                ...
                tile_factor = 1  # Track the tiling factor for later use with warped_noise
                if args.training_with_video_token_length:
                ...
                # Also tile warped_noise if present
                if args.use_warped_noise and 'warped_noise' in batch and tile_factor > 1:
                    if accelerator.is_main_process:
                        print(f"[WARPED NOISE TILING] Tiling warped_noise by factor {tile_factor}")
                        print(f"[WARPED NOISE TILING] Before: {batch['warped_noise'].shape}")
                    batch['warped_noise'] = torch.tile(batch['warped_noise'], (tile_factor, 1, 1, 1, 1))
                    if accelerator.is_main_process:
                        print(f"[WARPED NOISE TILING] After: {batch['warped_noise'].shape}")
                ...
                if accelerator.is_main_process:
                    print(f"[DEBUG] pixel_values shape BEFORE VAE encode: {pixel_values.shape}")

                with torch.no_grad():
                    # This way is quicker when batch grows up
                    def _batch_encode_vae(pixel_values):
                        if accelerator.is_main_process:
                            print(f"[DEBUG VAE] Input to VAE: {pixel_values.shape}")
                        pixel_values = rearrange(pixel_values, "b f c h w -> b c f h w")
                        if accelerator.is_main_process:
                            print(f"[DEBUG VAE] After rearrange: {pixel_values.shape}")
                        bs = args.vae_mini_batch
                        new_pixel_values = []
                        for i in range(0, pixel_values.shape[0], bs):
                            pixel_values_bs = pixel_values[i : i + bs]
                            if accelerator.is_main_process:
                                print(f"[DEBUG VAE] Encoding batch {i} to {i+bs}: {pixel_values_bs.shape}")
                            pixel_values_bs = vae.encode(pixel_values_bs)[0]
                            pixel_values_bs = pixel_values_bs.sample()
                            if accelerator.is_main_process:
                                print(f"[DEBUG VAE] Encoded output: {pixel_values_bs.shape}")
                            new_pixel_values.append(pixel_values_bs)
                        result = torch.cat(new_pixel_values, dim = 0)
                        if accelerator.is_main_process:
                            print(f"[DEBUG VAE] Final concatenated result: {result.shape}")
                        return result
                    if vae_stream_1 is not None:
                        ...
                        latents = _batch_encode_vae(pixel_values)
                    if accelerator.is_main_process:
                        print(f"[DEBUG] latents shape AFTER VAE encode (before scaling): {latents.shape}")
                    latents = latents * vae.config.scaling_factor
                    if accelerator.is_main_process:
                        print(f"[DEBUG] latents shape AFTER scaling: {latents.shape}")

                    if args.train_mode != "normal":
                        mask = 1 - mask
                        if not args.use_vae_mask and not args.shuffle_mask:
                            mask = rearrange(mask, "b f c h w -> b c f h w")
                            mask = resize_mask(mask, latents)

                            if args.binarize_mask:
                                if args.train_mode == "void":
                                    mask = torch.where(mask < 0.25, 0., mask).to(mask.dtype)
                                    mask = torch.where((mask >= 0.25) * (mask <= 0.75), 0.5, mask).to(mask.dtype)
                                    mask = torch.where(mask > 0.75, 1.0, mask).to(mask.dtype)
                                else:
                                    mask = torch.where(mask < 0.9, 0., 1.).to(mask.dtype)
                        elif args.shuffle_mask:
                            mask = rearrange(mask, "b f c h w -> b c f h w")
                            mask = torch.concat([
                            ...
                            # Ensure mask is in correct dtype for VAE
                            mask_for_vae = mask.repeat(1, 1, 3, 1, 1)
                            # VAE expects the same dtype as pixel_values
                            mask_for_vae = mask_for_vae.to(dtype=pixel_values.dtype, device=pixel_values.device)
                            mask = _batch_encode_vae(mask_for_vae)
                    ...
                    if accelerator.is_main_process:
                        print(f"[DEBUG] latents shape AFTER rearrange: {latents.shape}")
                ...
                if accelerator.is_main_process:
                    print(f"[DEBUG] latents shape AFTER stream wait: {latents.shape}")
                ...
                if accelerator.is_main_process:
                    ...
                    print(f"latents format should be (B, F, C, H, W) after rearrange at line 1697")

                # Sample noise that we'll add to the latents
                # Randomly choose between warped noise and random noise based on probability
                use_warped_this_batch = args.use_warped_noise and random.random() < args.warped_noise_probability

                if use_warped_this_batch and 'warped_noise' in batch:
                    # Use warped noise from Go-with-the-Flow
                    # Load warped noise from batch (shape: B, F, C, H, W in PIXEL space)
                    warped_noise_pixel = batch['warped_noise'].to(device=latents.device, dtype=weight_dtype)

                    if accelerator.is_main_process:
                        print(f"\n=== WARPED NOISE (PIXEL SPACE) ===")
                        print(f"Pixel-space warped noise shape: {warped_noise_pixel.shape}")

                    # Crop temporal frames to match actual_video_length (same as pixel_values)
                    # actual_video_length was computed earlier based on token_length sampling
                    if warped_noise_pixel.shape[1] > pixel_values.shape[1]:
                        warped_noise_pixel = warped_noise_pixel[:, :pixel_values.shape[1], :, :, :]
                        if accelerator.is_main_process:
                            print(f"Cropped warped noise temporally: {warped_noise_pixel.shape}")

                    # Manually downsample to latent space (can't VAE-encode 16-channel noise)
                    # Warped noise has C=16 channels (from GwF), not C=3 RGB
                    with torch.no_grad():
                        b, f, c, h, w = warped_noise_pixel.shape

                        # Spatial downsampling: mean-pool by 8x8 (VAE spatial compression factor)
                        noise_spatial = F.avg_pool2d(
                            rearrange(warped_noise_pixel, 'b f c h w -> (b f c) 1 h w'),
                            kernel_size=8,
                            stride=8
                        )
                        noise_spatial = rearrange(noise_spatial, '(b f c) 1 h w -> b f c h w', b=b, f=f, c=c)
                        noise_spatial = noise_spatial * 8.0  # Scale to preserve variance (GwF paper method)

                        # Temporal downsampling: subsample by 4x (VAE temporal compression factor)
                        target_frames = (f - 1) // 4 + 1
                        if f != target_frames:
                            indices = torch.linspace(0, f - 1, target_frames).long()
                            noise = noise_spatial[:, indices, :, :, :]
                        else:
                            noise = noise_spatial

                    if accelerator.is_main_process:
                        print(f"\n=== WARPED NOISE (LATENT SPACE) ===")
                        print(f"After spatial downsample (÷8): {noise_spatial.shape}")
                        print(f"After temporal downsample (÷4): {noise.shape}")
                        print(f"Target latents shape: {latents.shape}")

                    # Verify shape matches
                    if noise.shape != latents.shape:
                        if accelerator.is_main_process:
                            print(f"[ERROR] Shape mismatch after VAE encoding!")
                            print(f"  Noise: {noise.shape}")
                            print(f"  Latents: {latents.shape}")
                            print(f"[FALLBACK] Using random Gaussian noise")
                        noise = torch.randn(latents.size(), device=latents.device, generator=torch_rng, dtype=weight_dtype)
                    else:
                        if accelerator.is_main_process:
                            print(f"[SUCCESS] Warped noise matches latents perfectly!")

                        # Apply degradation: interpolate with random Gaussian noise
                        if args.warped_noise_degradation > 0:
                            gamma = torch.rand(bsz, 1, 1, 1, 1, device=latents.device, dtype=weight_dtype) * args.warped_noise_degradation
                            zeta = torch.randn_like(noise)
                            noise = (torch.sqrt(1 - gamma) * noise + zeta * torch.sqrt(gamma)) / torch.sqrt((1 - gamma)**2 + gamma**2)
                            if accelerator.is_main_process:
                                print(f"Applied degradation with gamma max={args.warped_noise_degradation}")

                        if accelerator.is_main_process:
                            print(f"[NOISE SOURCE] Using WARPED noise (probability={args.warped_noise_probability})")
                else:
                    # Standard random Gaussian noise (fallback if warped noise not available)
                    noise = torch.randn(latents.size(), device=latents.device, generator=torch_rng, dtype=weight_dtype)
                    if accelerator.is_main_process:
                        if use_warped_this_batch:
                            print(f"[NOISE SOURCE] FALLBACK to random Gaussian (warped noise not in batch)")
                        else:
                            print(f"[NOISE SOURCE] Using RANDOM Gaussian noise (warped probability={args.warped_noise_probability})")
