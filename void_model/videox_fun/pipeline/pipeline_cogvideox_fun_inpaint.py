...
class CogVideoXFunInpaintPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if is_wrapper:
            ...
            latents=mask_video["samples"]
        if comfyui_progressbar:
            pbar.update(1)
        mask_condition=None
        if mask_video is not None and not is_wrapper:
            ...
            inpaint_latents = mask_video["inpaint_latents"]
        else:
            if num_channels_transformer != num_channels_latents:
        ...
        if output_type == "numpy":
            ...
            video = latents 
        print(f"[CogVideoXFunPipeline] decoded video shape: {video.shape}") #[CogVideoXFunPipeline] decoded video shape: torch.Size([1, 22, 16, 48, 84])
        # Offload all models
        self.maybe_free_model_hooks()
