...
from ..perflib.fused import addmm_act
from ..sam.rope import apply_rotary_enc_real, VisionRotaryEmbeddingVE
...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.drop1(x)
        x = self.norm(x)
        if x.dtype != self.fc2.weight.dtype:  # sometimes  causes dtype issues with bfloat16
