...
class Sam3Processor:
    ...
    def set_text_prompt(self, prompt: str, state: Dict):
        ...
        # print(f"text_outputs keys: {list(text_outputs.keys())}")  # ['language_features', 'language_mask', 'language_embeds']
    ...
    def _forward_grounding(self, state: Dict):
        ...
        # print(f"outputs keys: {list(outputs.keys())}")  # ['pred_boxes', 'pred_logits', 'pred_masks', 'presence_logit_dec']
        out_bbox = outputs["pred_boxes"]
        out_logits = outputs["pred_logits"]
        out_masks = outputs["pred_masks"]
        out_probs = out_logits.sigmoid()
        presence_score = outputs["presence_logit_dec"].sigmoid().unsqueeze(1)
        # print(f"out_probs shape: {out_probs.dtype}, presence_score shape: {presence_score.dtype}") #out_probs shape: torch.Size([1, 100, 1]), presence_score shape: torch.Size([1, 1])
        ...
        # print(scale_fct.dtype, boxes.dtype) #torch.float32 torch.float32
