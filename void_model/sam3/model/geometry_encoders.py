...
class SequenceGeometryEncoder(nn.Module):
    ...
    def _encode_points(self, points, points_mask, points_labels, img_feats):
        ...
        if self.points_pool_project is not None:
            ...
            proj = self.points_pool_project(sampled) ##torch.bfloat16
        ...
        if self.points_pos_enc_project is not None:
            ...
            enc = torch.cat([enc_x, enc_y], -1) #float32
            if enc.dtype != img_feats.dtype:
                enc = enc.to(img_feats.dtype)
            proj = self.points_pos_enc_project(enc) #torch.bfloat16
            if points_embed is None:
                points_embed = proj
            else:
                points_embed = points_embed + proj

        type_embed = self.label_embed(points_labels.long()) #torch.bfloat16
        if points_embed.dtype != img_feats.dtype:
            points_embed = points_embed.to(img_feats.dtype)
    ...
    def _encode_boxes(self, boxes, boxes_mask, boxes_labels, img_feats):
        ...
        if self.boxes_pool_project is not None:
            ...
            origin_dtype = img_feats.dtype
            if boxes_xyxy.dtype != img_feats.dtype:
                boxes_xyxy = boxes_xyxy.to(img_feats.dtype)
            ...
            sampled=sampled.to(origin_dtype)
        ...
        if boxes_embed.dtype != img_feats.dtype:
            boxes_embed = boxes_embed.to(img_feats.dtype)
    ...
    def _encode_masks(
        ...
        if masks.dtype != img_feats.dtype:
            print(f"Warning: masks dtype {masks.dtype} is different from img_feats dtype {img_feats.dtype}. Casting masks to {img_feats.dtype}")
            masks = masks.to(img_feats.dtype)
    ...
    def forward(self, geo_prompt: Prompt, img_feats, img_sizes, img_pos_embeds=None):
        ...
        # print(f"final_embeds shape: {final_embeds.shape}, final_mask shape: {final_embeds.dtype}") #final_embeds shape: torch.Size([1, 1, 256]), final_mask shape: torch.bfloat16
        if self.encode is not None:
            if final_embeds.dtype != seq_first_img_feats.dtype:
