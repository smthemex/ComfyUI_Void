...
class Sam3Image(torch.nn.Module):
    ...
    def _encode_prompt(
       ...
       # print(f"txt_feats shape: {txt_feats.dtype}, txt_masks shape: {txt_masks.dtype}") #txt_feats shape: torch.bfloat16, txt_masks shape: torch.bool
        feat_tuple = self._get_img_feats(backbone_out, find_input.img_ids)
        
        backbone_out, img_feats, img_pos_embeds, vis_feat_sizes = feat_tuple
        #print(f"img_feats[0] shape: {img_feats[0].dtype}, img_pos_embeds[0] shape: {img_pos_embeds[0].dtype}") #img_feats[0] shape: torch.bfloat16, img_pos_embeds[0] shape: torch.bfloat16
        if prev_mask_pred is not None:
            img_feats = [img_feats[-1] + prev_mask_pred]
        # Encode geometry
        #print(f"Encoding geometric prompt with dtype {geometric_prompt.box_embeddings.dtype}") #torch.float32
        target_dtype = img_feats[0].dtype
        if geometric_prompt.box_embeddings is not None:
            geometric_prompt.box_embeddings = geometric_prompt.box_embeddings.to(target_dtype)
        if geometric_prompt.point_embeddings is not None:
            geometric_prompt.point_embeddings = geometric_prompt.point_embeddings.to(target_dtype)
        if geometric_prompt.mask_embeddings is not None:
            geometric_prompt.mask_embeddings = geometric_prompt.mask_embeddings.to(target_dtype)
        #Encoding geometric prompt with dtype torch.float32
        ...
        #print(f"geo_feats shape: {geo_feats.dtype}, geo_masks shape: {geo_masks.dtype}") #geo_feats shape: torch.bfloat16, geo_masks shape: torch.bool
        if geo_feats.dtype!=txt_feats.dtype:
            #print(f"⚠️  Casting geo_feats to {txt_feats.dtype} to match txt_feats dtype")
            geo_feats = geo_feats.to(txt_feats.dtype)
        ...
        if encode_text:
            #print(f"Encoding text prompt with dtype {txt_feats.dtype}, geometric prompt with dtype {geo_feats.dtype}, visual prompt with dtype {visual_prompt_embed.dtype}") # Encoding text prompt with dtype torch.bfloat16, geometric prompt with dtype torch.bfloat16, visual prompt with dtype torch.bfloat16
            ...
            #print(f"Encoding geometric prompt with dtype {geo_feats.dtype}, visual prompt with dtype {visual_prompt_embed.dtype}") # Encoding geometric prompt with dtype torch.bfloat16, visual prompt with dtype torch.bfloat16
    ...
    def _run_encoder(
        ...
        feat_tuple = self._get_img_feats(backbone_out, find_input.img_ids)
        backbone_out, img_feats, img_pos_embeds, vis_feat_sizes = feat_tuple
        #print(f"Running encoder with img_feats dtype {img_feats[0].dtype}, prompt dtype {prompt.dtype}, prompt_mask dtype {prompt_mask.dtype}") # Running encoder with img_feats dtype torch.bfloat16, prompt dtype torch.bfloat16, prompt_mask dtype torch.bool
        #Running encoder with img_feats dtype torch.bfloat16, prompt dtype torch.bfloat16, prompt_mask dtype torch.bool
        # Run the encoder
        prompt_pos_embed = torch.zeros_like(prompt).to(prompt.dtype)
        #print(f"prompt shape: {prompt.dtype}, prompt_pos_embed shape: {prompt_pos_embed.dtype}") #prompt shape: torch.bfloat16, prompt_pos_embed shape: torch.bfloat16
    ...
    def _run_decoder(
        ...
        bs = memory.shape[1]
        query_embed = self.transformer.decoder.query_embed.weight #torch.bfloat16
        if query_embed.dtype != memory.dtype:
            query_embed = query_embed.to(memory.dtype)
        tgt = query_embed.unsqueeze(1).repeat(1, bs, 1)
        #print(f"Running decoder with memory dtype {memory.dtype}, query_embed dtype {query_embed.dtype}, pos_embed dtype {pos_embed.dtype}, prompt dtype {prompt.dtype}, prompt_mask dtype {prompt_mask.dtype}")
        ...
        #print(f"Decoder output hs dtype: {hs.dtype}, reference_boxes dtype: {reference_boxes.dtype}") # Decoder output hs dtype: torch.float16, reference_boxes dtype: torch.float16
        #Decoder output hs dtype: torch.bfloat16, reference_boxes dtype: torch.bfloat16
    ...
    def _update_scores_and_boxes(
        ...
        if self.use_dot_prod_scoring:
            ...
            target_dtype = dot_prod_scoring_head.prompt_proj.weight.dtype
            if hs.dtype != target_dtype:
