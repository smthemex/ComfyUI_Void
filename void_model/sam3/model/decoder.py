...
class TransformerDecoder(nn.Module):
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        ...
        if boxes_xyxy.dtype != reference_boxes.dtype:
            boxes_xyxy = boxes_xyxy.to(reference_boxes.dtype)
        ...
        # 确保coords与reference_boxes类型一致
        coords_h = coords_h.to(reference_boxes.dtype)
        coords_w = coords_w.to(reference_boxes.dtype)
        ...
        mlp_dtype = self.boxRPB_embed_x.layers[0].weight.dtype
        deltas_x = deltas_x.to(mlp_dtype)
        deltas_y = deltas_y.to(mlp_dtype)
    ...
    def forward(
        ...
        if self.box_refine:
            if reference_boxes is None:
                ...
                reference_boxes = self.reference_points.weight.unsqueeze(1) #torch.bfloat16
                reference_boxes=reference_boxes.to(tgt.dtype) 
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            #print(f"query_sine_embed dtype: {query_sine_embed.dtype}, reference_points_input dtype: {reference_points_input.dtype}, valid_ratios dtype: {valid_ratios.dtype}") #query_sine_embed dtype: torch.float32, reference_points_input dtype: torch.float16, valid_ratios dtype: torch.float16
            #query_sine_embed dtype: torch.float32, reference_points_input dtype: torch.float32, valid_ratios dtype: torch.float32
            # conditional query
            if query_sine_embed.dtype != self.ref_point_head.layers[0].weight.dtype:
