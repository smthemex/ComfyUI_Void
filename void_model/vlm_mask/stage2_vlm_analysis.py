...
def call_vlm_with_images_and_video(client, model: str, image_data_urls: list,
                                    video_data_url: str, prompt: str,pass_video=False) -> str:
...
def parse_vlm_response_(raw: str) -> Dict:
    ...
    for item in parsed.get("affected_objects", [])[:5]:
        obj = {
            ...
            "first_appears_frame": 0,  # Default value
            "movement_description": str(item.get("movement_description", "")).strip()[:300]
        }

        # Parse first_appears_frame with error handling
        first_appears = item.get("first_appears_frame", 0)
        try:
            if isinstance(first_appears, str):
                if first_appears.lower() == "unknown":
                    obj["first_appears_frame"] = 0  # Default to 0 if unknown
                else:
                    try:
                        obj["first_appears_frame"] = int(first_appears)
                    except ValueError:
                        obj["first_appears_frame"] = 0  # Default to 0 if conversion fails
            else:
                obj["first_appears_frame"] = int(first_appears)
        except (ValueError, TypeError):
            obj["first_appears_frame"] = 0  # Default to 0 if any error occurs
    ...
    try:
        try:
            raw_response = call_vlm_with_images_and_video(
                client, model, image_data_urls, video_data_url, prompt,True if model!="gemini-3-pro-preview" else False
            )
        except Exception as e:
            # If multi-frame fails (likely payload size issue), fall back to single frame
            if use_multi_frame_grids and "400" in str(e):
                print(f"   ⚠️  Multi-frame request failed (payload too large?){e}")
        ...
        print(f"   Parsing VLM response...{raw_response}")

        if model!="gemini-3-pro-preview":
            analysis = parse_vlm_response_(raw_response)
        else:
...
def process_config(config_path: str, model: str = DEFAULT_MODEL,vllm_model="qwen3.5:9b"):
    ...
    if len(videos) >0:
        if  os.path.exists(os.path.join(videos[0].get("output_dir", ""), "vlm_analysis.json")):
            print(f"VLM analysis already generated for first video, skipping Stage 2")
            return
    ...
    if  api_key is  None:
        client = openai.OpenAI(
            ...
            api_key="localhost"  # 任意字符串，本地调用无需密钥
        )
        model=vllm_model
